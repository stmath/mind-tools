...
var optionDefinitions = [{ name: 'upload', alias: 'u', type: Boolean }, { name: 'dest', alias: 'd', type: String, defaultValue: 'components/' }, { name: 'version', type: String }, { name: 'tag', type: Boolean }];
...
var success = true; //bundleComponents(version, {sourceMap: options.sourceMap, noMinify: options.noMinify});
if (!success) new Error('Error while bundling Components.');
if (options.upload) {
	log('Upload bundle: ' + version);
	(0, _bundle.uploadBundleComponents)(version);
}
if (options.tag) {
